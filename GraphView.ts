...
export class GraphView extends ItemView {
  ...
  async onOpen() {
    // defensive add of class for theming
    try { this.contentEl.addClass('greater-graph-view'); } catch (e) {}
    ...
    if (this.controller) this.controller.setNodeClickHandler((node: any) => void this.openNodeFile(node));
  }
  ...
  private async openNodeFile(node: any): Promise<void> {
    ...
    try { await leaf.openFile(file); } catch (e) {}
  }
}
...
class Graph2DController {
  ...
  async init(): Promise<void> {
    ...
    if (this.renderer && this.graph) this.renderer.setGraph(this.graph);
    if (this.graph) {
    ...
    if (this.renderer) this.renderer.resize(rect.width || 300, rect.height || 200);
  ...
  private hitTestNode(x: number, y: number) {
    ...
    for (const node of this.graph.nodes) {
      ...
      const nodeRadius = (this.renderer as any).getNodeRadiusForHit ? (this.renderer as any).getNodeRadiusForHit(node) : 8;
  ...
  handleHover(x: number, y: number): void {
    ...
    if (this.renderer && (this.renderer as any).setHoveredNode) this.renderer.setHoveredNode(newId);
    if (this.renderer) this.renderer.render();
  }

  clearHover(): void { if (!this.renderer) return; if ((this.renderer as any).setHoverState) (this.renderer as any).setHoverState(null, new Set(), 0, 0); if (this.renderer && (this.renderer as any).setHoveredNode) this.renderer.setHoveredNode(null); if (this.renderer) this.renderer.render(); }
}
import { App, ItemView, WorkspaceLeaf, Plugin, TFile } from 'obsidian';
import { buildGraph, GraphData } from './graph/buildGraph';
import { layoutGraph2D } from './graph/layout2d';
...
export class GraphView extends ItemView {
  ...
  async onOpen() {
    this.containerEl.empty();
    // Ensure the view container matches Obsidian theming
    try { this.contentEl.addClass('greater-graph-view'); } catch (e) { /* graceful fallback */ }
    const container = this.containerEl.createDiv({ cls: 'greater-graph-view' });
    this.controller = new Graph2DController(this.app, container, this.plugin);
    await this.controller.init();
    if (this.controller) {
