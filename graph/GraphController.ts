...
import { Settings, Renderer, GraphData, GraphNode, Simulation} from '../utilities/Interfaces.ts';
  ...
  private recreateSimulation(showTags: boolean, extraOpts?: { centerX?: number; centerY?: number; centerNodeId?: string }) {
    ...
    try {
      ...
      if (!showTags) {
        ...
        nodes = this.graph.nodes.filter((n: any) => {
          if ((n as any).type === 'tag') { tagSet.add(n.id); return false; }
          return true;
        });
        edges = (this.graph.edges || []).filter((e: any) => !tagSet.has(e.sourceId) && !tagSet.has(e.targetId));
      }

      this.simulation = createSimulation(nodes, edges);
      // start simulation
